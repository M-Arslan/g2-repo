...
import {
    CreateClaimDrawer
} from './CreateClaimDrawer/CreateClaimDrawer';
import {
...
import { loadClaimGridData, getDJClaimID } from './Queries/loadClaimGridData';
...
export const Dashboard = ({ match, landingPage = null }) => {
    const $auth = useSelector(userSelectors.selectAuthContext());
    const $dispatch = useDispatch();

    const isViewer = $auth.isReadOnly(APP_TYPES.Claim_Dashboard);
        ...
        return {
            getRows: async function (params) {
                ...
                let searchDataObj = {
                    ...
                    "claimTypeText": "Casualty,Property,NULL"
                };
                let claimSearch = "claimSearch";
                if ((sortCount > 0 || filterCount > 0) && (selectedView === undefined)) {
      ...
      <>
            { isAuthorize ? (
                <ClaimLandingContainer>
                    <ClaimLandingToolbar>
                        <Toolbuttons>
                            {(isViewer !== true && (($auth.defaultCompany.G2LegalEntityID !== LEGAL_ENTITY.GENERAL_STAR && otherCompany === true) || ($auth.defaultCompany.G2LegalEntityID === LEGAL_ENTITY.GENERAL_STAR && otherCompany === false))) ?
                    ...
                    <CreateClaimDrawer isOpen={createOpen} onCloseDrawer={setCreateOpen} screenName={landingPage} createdClaim={createdClaimFunction} />
                    { users?.length > 0 ? (
