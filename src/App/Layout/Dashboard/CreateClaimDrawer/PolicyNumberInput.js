...
export const PolicyNumberInput = forwardRef((params, ref) => {
    ...
    const changeStatusTo = (newStatus) => {
        if (status !== newStatus) {
            if (typeof params.onSearchStatusChanged === 'function' && newStatus !== STATUS.SEARCHING) {
                params.onSearchStatusChanged({
                    previous: status,
                    current: newStatus,
                });
            }
    ...
    const onSearchClick = async () => {
        ...
        try {
            const query = {
                "query": `
                    query {
                       policy(id:"${policyNumber}", g2LegalEntityID:${parseInt(params.g2LegalEntityID)}){
                                insuredName
            ...
            if (typeof policy === 'object' && policy !== null) {
                ...
                if (typeof params.onChange === 'function') {
                    params.onChange({ target: { name: 'claimPolicy', value: policy } });
                }
                setError(false);
            }
            else {
