...
import { useGraphQL } from '../../../../../../Core/Providers/GraphQL/useGraphQL';
...
export const DownloadModal = ({ open, onClose }) => {
    ...
    const startDownload = () => {
        ...
        $api.download({ claimMasterID: $host.claimMasterId, claimID: $host.claim.claimID }).then(result => {
                ...
                <Panel height="100%" width="100%">
                    ...
                    <PanelContent className={classes.panelContent}>
                        <CenteredRow height="20%">
                            <p>{getStageMessage()}</p>
                            {
                                downloading === DL_STATUS.COMPLETE ? <Button id="download-pdf" download={`${$host.claim.claimID} - Activity Log.pdf`} href={pdfData} variant="contained" color="primary">Click Here to Download PDF Export</Button>
