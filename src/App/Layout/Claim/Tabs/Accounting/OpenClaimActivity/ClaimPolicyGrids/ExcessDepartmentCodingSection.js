...
export const ExcessDepartmentCodingSection = ({ request,policyID, onRowSelected }) => {
    ...
    const loadData = async () => {
        let query = {
            "query": `
            query {
               excessPolicyCodings(policyID:"${policyID}"){ 
                   aggregateLimit
                    attachmentTypeCode
                    classCode
                    className
        ...
        let covereagesData = []
        covereagesData = result.data.excessPolicyCodings || [];
